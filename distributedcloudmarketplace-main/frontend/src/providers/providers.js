...
const DataUpload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (importType === 'data' || importType === 'application') {
        const response2 = await axios.post('http://localhost:5000/content/upload', formData, {
          headers: { 
            'Content-Type': 'multipart/form-data',
            'Authorization': `Bearer ${token}`
          },
      ...
      // Envoi à l'endpoint /content/upload pour data et application
      // if (importType === 'data' || importType === 'application') {
      //   await formDataBis(file.name, importType, token);
      // }
        } catch (error) {
      ...
      // const formDataBis = async (fileName, fileType, token) => {
      //   try {
      //     const data = { fileName, fileType };
      //     console.log('Sending request to: http://localhost:5000/content/upload with data:', data);
      //     const response = await axios.post('http://localhost:5000/content/upload', data, {
      //       headers: { 
